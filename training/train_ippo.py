"""
Training script for Independent PPO comparison.
Uses IndependentPPOTrainer instead of MAPPOTrainer (no parameter sharing).
Usage: python3 training/train_ippo.py --config configs/configN.yaml --output results/ippo/ --seeds 3
"""
...
def run_episode(env: ReputationMARLEnv, trainer: IndependentPPOTrainer,
...
def train_config(cfg: dict, seed: int, output_dir: Path) -> dict:
    ...
    # IndependentPPOTrainer takes agent_ids in constructor
    ...
    while ep < hard_limit:
        ...
        if ep % 100 == 0:
            ...
            print(f"  [{cfg['name']} IPPO seed={seed}] ep={ep} "
        ...
        if ep >= max_ep and not converged:
            if check_convergence(reward_history, conv_window, conv_thresh):
                converged = True
                converged_at = ep
                print(f"  [{cfg['name']} IPPO seed={seed}] CONVERGED at ep={ep}")
                break
        ...
        if time.time() - start_time > wall_budget:
            print(f"  [{cfg['name']} IPPO seed={seed}] Wall-clock budget exceeded at ep={ep}")
            break
...
def main():
    ...
    parser.add_argument("--output", default="results/ippo/")
    ...
    print(f"\n{'='*60}")
    print(f"IPPO Training: {cfg['name']}")
