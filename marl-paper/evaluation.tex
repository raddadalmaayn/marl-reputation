...
colluding subgroup (4 members) with Sybil capability.  Results from 5
completed seeds show:
\begin{itemize}[noitemsep]
  \item Honest \% = $75.84\% \pm 38.38\%$ (highest variance of any configuration)
  \item Defense block rate = $48.0\%$ (72{,}507/164{,}345 attempts)
  \item Mean reward = $-2.24 \pm 1.89$ (most negative; high attack costs)
\end{itemize}
The high standard deviation ($38.4\%$) indicates bimodal seed outcomes: some
...
provides only a $48\%$ block rate, and honest convergence is not reliable
...
Eight of the 11 configurations show statistically significant honest behavior
above the 95\% threshold ($p < 0.05$).  The three exceptions are C3 (Sybil
flooding), C6 (self-rating), and C11 (comprehensive), which exhibit high
cross-seed variance arising from non-convergent seeds.  For C11, the wide
confidence interval ($75.8\% \pm 38.4\%$) spans both failure and success
modes, preventing a significant aggregate result despite some seeds achieving
near-perfect honest behavior.
