...
The comprehensive multi-vector configuration (C11) identifies the primary
open challenge: extended training to 30{,}000 episodes achieves full
convergence but reveals a strongly bimodal equilibrium --- 4 of 5 seeds
converge to an adversarial-dominant basin ($0\%$ honest) and 1 of 5 to the
honest Nash equilibrium ($100\%$ honest), yielding an aggregate honest rate
of $20.0\% \pm 40.0\%$ and a 48.0\% defense hold rate across 164{,}345
attack attempts.  An ablation study confirms that stake-based economic deterrence is the single most critical defense mechanism --- removing it degrades honest behavior below 90\% --- while parameter sensitivity analysis shows the system maintains $>95\%$ honest behavior for adversarial incentive bonuses up to $b=1.0$ (2$\times$ the baseline).  All source code, configurations, and training
...
\begin{itemize}[noitemsep]
  ...
  \item Identification of the \emph{comprehensive multi-vector attack} (C11)
        as the primary open challenge: full convergence (5/5 seeds) requires
        up to 16{,}593 episodes and reveals a bimodal equilibrium --- 4/5
        seeds settle adversarial-dominant; aggregate honest rate $20.0\%$,
        48.0\% defense hold rate across 164{,}345 attack attempts
